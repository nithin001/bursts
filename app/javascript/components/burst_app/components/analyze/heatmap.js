...
export default function createHeatMap(chart, months, data, startDate, endDate) {
  ...
  rect
    ...
    .attr("title", (d) => {
      ...
      if (!countData || !countData.count) return `No burst on ${date}`;
      else if (countData.count === 1) return `1 burst on ${date} taking ${countData.humanized_time_spent}`;
      else return `${countData.count} bursts on ${date} taking ${countData.humanized_time_spent}`;
    })
    .attr("date", (d) => d)
